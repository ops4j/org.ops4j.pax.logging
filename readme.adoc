...
* pax-logging-log4j1 (log4J1)
* pax-logging-log4j2 (log4J2)
* pax-logging-logback (logoback)

`pax-logging-log4j1` was named `pax-logging-service` before version 2.0.0.
...
`slf4j-api-1.7.29-sources.jar` contains more sources than `slf4j-api-1.7.29.jar` has classes - in particular, `org.slf4j.impl` package is removed from the jar and the responsibility to provide:
...
* `slf4j-nop-1.7.29.jar`
* `slf4j-log4j12-1.7.29.jar`
* `slf4j-simple-1.7.29.jar`

pax-logging-api provides own implementation of these three classes. All other classes are directly repackaged (using bndlib) from `slf4j-api-1.7.29.jar` - classes that don't have to be changed are no longer shipped in pax-logging-api source directory.
...
`pax-logging-log4j1` itself should not export anything (it's not doing it so far).
...
For completeness, when pax-logging-log4j1 is used through `org.osgi.service.log.LogService` API, the stack trace is:
[listing,options=nowrap]
----
"Karaf Shell Console Thread@9234" daemon prio=5 tid=0x30 nid=NA runnable
  java.lang.Thread.State: RUNNABLE
	  ...
	  at org.ops4j.pax.logging.log4j1.internal.PaxLoggerImpl.doLog(PaxLoggerImpl.java:145)
	  at org.ops4j.pax.logging.log4j1.internal.PaxLoggerImpl.inform(PaxLoggerImpl.java:179)
	  at org.ops4j.pax.logging.log4j1.internal.PaxLoggingServiceImpl.log(PaxLoggingServiceImpl.java:167)
	  at org.ops4j.pax.logging.log4j1.internal.PaxLoggingServiceImpl.access$000(PaxLoggingServiceImpl.java:48)
	  at org.ops4j.pax.logging.log4j1.internal.PaxLoggingServiceImpl$1ManagedPaxLoggingService.log(PaxLoggingServiceImpl.java:331)
