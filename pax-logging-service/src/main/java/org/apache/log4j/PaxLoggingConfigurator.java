...
public class PaxLoggingConfigurator extends PropertyConfigurator {
    ...
    Appender parseAppender(Properties props, String appenderName) {
        ...
        if (appenderName.startsWith(OSGI_PREFIX)) {
            // pax-logging-service: appender is not a class name - it's part of OSGi service filter.
            ...
            if (appender instanceof OptionHandler) {
                if (appender.requiresLayout()) {
                    ...
                    if (layoutClass != null && layoutClass.startsWith(OSGI_PREFIX)) {
                        // pax-logging-service: layout is not a class name - it's part of OSGi service filter.
                        String osgiLayoutName = layoutClass.substring(OSGI_PREFIX.length());
                        Layout fallback = null;
                        // pax-logging-service: log4j.appender.appenderName.layout.fallback may specify fallback class for layout
                ...
                if (errorHandlerClass != null && errorHandlerClass.startsWith(OSGI_PREFIX)) {
                    // pax-logging-service: error handler is not a class name - it's part of OSGi service filter.
                    String errorHandlerName = errorHandlerClass.substring(OSGI_PREFIX.length());
                    ErrorHandler fallback = null;
                    // pax-logging-service: log4j.appender.appenderName.errorhandler.fallback may specify fallback class for error handler
    ...
    void parseAppenderFilters(Properties props, String appenderName, Appender appender) {
        ...
        while (g.hasMoreElements()) {
            ...
            if (clazz != null && clazz.startsWith(OSGI_PREFIX)) {
                // pax-logging-service: filter is not a class name - it's part of OSGi service filter.
                String filterName = clazz.substring(OSGI_PREFIX.length());
                Filter fallback = null;
                // pax-logging-service: log4j.appender.appenderName.filter.fallback may specify fallback class for filter
